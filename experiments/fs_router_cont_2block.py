"""
Depth-N (default 2) stack of the router-continuous fully-spiking block.
Each block operates in C-space with an intra-block residual:  h <- h + Block(h).
Block (router-continuous fully-spiking):
  hs = QIF(h) ; g = softmax(router(h)) [continuous] ; mix = sum(g*experts(hs)) -> QIF -> down -> QIF
Lift c1 (continuous) -> [Block]*N -> readout c2 ; u^{t+1} = u^t + c2(h).

Usage: python fs_router_cont_2block.py --blocks 2
"""
...
class Block(nn.Module):
    """router-continuous fully-spiking block, C-space in/out."""
    def __init__(s,C,mode,ratio,experts,top_k):
    ...
    def forward(s,h,mems):
        m1,mh,md=mems if mems else (None,None,None)
        hs,m1=s.q1(h,m1)
        if s.mode=='dense':
            z,mh=s.qh(s.up(hs),mh); g=None
        else:
            g=F.softmax(s.router(h),dim=1)                          # gate from CONTINUOUS h
            if s.K<s.E:
...
class Net(nn.Module):
    def __init__(s,n_ch,C,mode,ratio,experts,top_k,nblocks,kernel=5):
        super().__init__(); pad=kernel//2; s.nblocks=nblocks
        s.c1=nn.Conv1d(n_ch,C,kernel,padding=pad)
        s.blocks=nn.ModuleList([Block(C,mode,ratio,experts,top_k) for _ in range(nblocks)])
        s.c2=nn.Conv1d(C,n_ch,kernel,padding=pad)
    def step(s,u,mems):
        if mems is None: mems=[None]*s.nblocks
        h=s.c1(u); nm=[]; g_last=None
        for i,blk in enumerate(s.blocks):
            d,mi,g=blk(h,mems[i]); h=h+d; nm.append(mi); g_last=g
        return u+s.c2(h), nm, g_last
...
def train(mode,ratio,nblocks,cfg):
    torch.manual_seed(cfg.seed); np.random.seed(cfg.seed)
    U,U_te,m,sd=get_data(cfg); U=U.to(dev)
    model=Net(U.shape[2],cfg.C,mode,ratio,cfg.experts,cfg.top_k,nblocks).to(dev)
    ...
    for ep in range(cfg.epochs):
        ...
        if ep%300==0: print(f'  [{mode} x{nblocks}] ep{ep} loss={loss.item()/(cfg.nt_train-1):.4e}',flush=True)
    rel=relL2(model,U_te.to(dev),cfg.nt_long)
    print(f'[fs-rc-{nblocks}blk/{mode}] params={count_params(model)} held-out relL2={rel:.4f}',flush=True)
    return rel,count_params(model)

if __name__=='__main__':
    ap=argparse.ArgumentParser(); ap.add_argument('--blocks',type=int,default=2); ap.add_argument('--epochs',type=int,default=None); a=ap.parse_args()
    cfg=copy.copy(PAPER)
    if a.epochs is not None: cfg.epochs=a.epochs
    rm,pm=train('moe',cfg.ratio,a.blocks,cfg); rd,pd=train('dense',cfg.dense_ratio,a.blocks,cfg)
    print(f'\nRESULT  {a.blocks}-block fully-spiking(router-cont) Burgers held-out relL2:',flush=True)
    print(f'   MoE={rm:.4f} ({pm}p)   dense={rd:.4f} ({pd}p)   ({"MoE wins" if rm<rd else "dense wins"})',flush=True)
    print('COMPARE 1-block: MoE=0.1441 dense=0.1649 (router-cont) | MoE=0.0971 dense=0.1325 (1-QIF)',flush=True)
